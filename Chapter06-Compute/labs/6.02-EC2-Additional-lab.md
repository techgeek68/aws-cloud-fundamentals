...
- Network settings:
  - VPC: default
  - Subnet: Use any public subnet in your chosen AZ
  ...
  - Security group: Create new, e.g.,
    ...
    - Inbound rules:
      - SSH, TCP 22, Source: 0.0.0.0/0                  [My IP (recommended)]
...
```
  ...
  - Windows (PowerShell)
    - icacls "filename" /inheritance:r                          # Remove Inheritance
    - icacls "filename" /grant:r "username:permission"          # Grant Read Permissions
...
**Build a Simple React Web App with Vite**
- Create a project
```bash
cd ~
